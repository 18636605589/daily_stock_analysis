...
"""
    ...
    def generate_market_review(self, overview: MarketOverview, news: List) -> str:
        ...
        first_call_duration_ms = int((time.perf_counter() - llm_started_at) * 1000)
        first_complete = self._is_review_complete(review)

        if not first_complete:
            logger.warning(
                "[大盘] %s action=generate_review status=retry reason=incomplete length=%d",
                self._log_context(),
                len(review or ""),
            )
            ...
            try:
                record_llm_run_started(
                    provider="litellm",
                    model=getattr(self.config, "litellm_model", None),
                    call_type="market_review_retry",
                )
                review = self.analyzer.generate_text(retry_prompt, max_tokens=8192, temperature=0.3)
            except Exception as exc:
                record_llm_run(
                    success=False,
                    provider="litellm",
            ...
            if review is not None:
                record_llm_run(
                    ...
                    error_message=None if review else "empty retry market review response",
                )

        is_complete = self._is_review_complete(review)
        if is_complete:
            logger.info(
                "[大盘] %s action=generate_review status=success length=%d retried=%s",
                self._log_context(),
                len(review or ""),
                not first_complete,
            )
            # Inject structured data tables into LLM prose sections
            return self._inject_data_into_review(review or "", overview, news)

        if first_complete is False and review is not None:
            record_llm_run(
                success=True,
                provider="litellm",
                model=getattr(self.config, "litellm_model", None),
                call_type="market_review",
                duration_ms=first_call_duration_ms,
                error_type="IncompleteResponse",
                error_message="first market review response was incomplete",
            )

        logger.warning(
            "[大盘] %s action=generate_review status=fallback_template reason=incomplete_llm_response",
            self._log_context(),
        )
        return self._generate_template_review(overview, news)
